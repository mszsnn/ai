...
def convert_to_one_hot(Y, C):
    """
        将一维的标签向量转换为 (C, m) 维度的 One-Hot 矩阵

        参数:
        Y -- 真实标签向量，维度为 (1, m)，包含类别索引 (0 到 C-1)
        C -- 总类别数

        返回:
        Y_one_hot -- 转换后的独热矩阵，维度为 (C, m)
        """
    # np.eye(C) 会生成一个 C x C 的单位对角矩阵
    # 我们用 Y 的值作为索引去“抽”出对应的行，最后转置(.T)以符合吴恩达 (C, m) 的标准
    Y_one_hot = np.eye(C)[Y.reshape(-1)].T
